...
class ProfileManager {
  ...
  async fetchProfileData() {
    try {
      const res = await fetch("api?route=Api/Patients/get-patient-profile");
  ...
  setGenderValue(value) {
    ...
    options.forEach((opt) => {
      ...
      if (selected)
        document.getElementById("gender-input").value = selectedValue;
    });
  }

  getSelectedGender() {
    const selected = this.genderInput.querySelector(
      '[data-dropdown="option"] svg:not(.hidden)'
    );
    if (!selected) return "";
    return (
      this.capitalize(selected.closest("button")?.getAttribute("data-value")) ||
      "Not specified"
    );
  }
  ...
  async submitForm() {
    ...
    try {
      ...
      const res = await fetch("api?route=Api/Patients/update-patient-profile", {
        method: "POST",
        body: formData,
      });
